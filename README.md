...
التطبيق يعمل بسمة واحدة ثابتة: **«الزمرّد»** — أبيض ناصع + أخضر غابيّ زمرّديّ (تدعم
الوضعين الفاتح والداكن حسب إعداد النظام). **لا توجد شاشة «المظهر» ولا سمات بديلة**؛
قِيَم السمة مدمجة مباشرةً في `:root` داخل `src/styles.css`، ولا يوجد `ThemeService`.

لتعديل ألوان الواجهة: عدّل كتلتَي `:root` (الفاتح + `@media dark`) الخاصّتين بالزمرّد أعلى
`src/styles.css`. شعار المركز في `public/logo.svg` — يظهر أعلى شاشة الدخول.

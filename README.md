...
تطبيق جوّال (Android / APK) لمعلّمي **الماهر** لتسجيل متابعة الطلاب:
...
> المرور تقبل 6 خانات بسيطة (مثل `123456`). شعار «الماهر» في `public/almaher-logo.jpg`.
...
**تصميمان قابلان للتبديل** من **حساب المعلّم ← المظهر** (يُحفَظ الاختيار محليًّا،
ويدعمان الوضعين الفاتح والداكن):

| التصميم | الطابع |
| --- | --- |
| **مِسْك الدافئ** (الافتراضيّ) | خلفية كريميّة دافئة `#F5F1E8` + أخضر غابيّ `#3B6B4A` + أزرق إردوازيّ `#3D5F72` + برونزيّ `#A07030` + خطّ **Cairo** + بطاقات ناعمة وزخرفة نجمة إسلامية خافتة — مستوحى من تصميم Figma |
| **الزمرّد الكلاسيكيّ** | التصميم السابق — أبيض ناصع + أخضر زمرّديّ `#0b6b46` + شريط ذهبيّ على بطاقات الإحصاء + خطّ **Tajawal** |

الآلية: `data-app-theme` على `<html>` (يضبطه `ThemeService` وسكربت الإقلاع في `index.html`).
قِيَم «الزمرّد» هي الرموز الأساسية في `:root`؛ و«مِسْك» كتلة `:root[data-app-theme='misk']`
في `src/styles.css`. شعار «الماهر» في `public/almaher-logo.jpg`.
...
```
npm run apk:debug      # يبني ويُخرج إلى apk/AlMaher-Teacher-debug-<التاريخ>.apk

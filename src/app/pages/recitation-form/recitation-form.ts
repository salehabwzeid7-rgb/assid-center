...
export class RecitationFormPage implements OnInit {
  ...
  async submit(): Promise<void> {
    ...
    if (this.m.kind === 'new') {
      ...
      // تنبيه اكتمال جزء → دعوة لتسجيل السرد واختبار الجزء (اختبار مستقلّ لكلّ جزء)
      if (completedJuz.length > 0) {
        const jz = completedJuz.map((j) => `الجزء ${j}`).join(' و');
        this.notify.info(
          `🎉 أكمل الطالب حفظ ${jz} — سجّل السرد من صفحة «السرد»، وافتح اختبار ${completedJuz.length > 1 ? 'كلّ جزء' : 'الجزء'} من صفحة «الاختبار».`,
        );
      }
    }
